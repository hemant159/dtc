...
const calculateShiftEndTime = (shift) => {
  ...
  let endTime;
  if (shift === "morning") {
    endTime = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 14, 0, 0); // 2:00 PM
  ...
  try {
    ...
    if (busCount > previousBusCount) {
      ...
      for (let i = 0; i < newBuses.length; i++) {
        ...
        const timeLeftInShift = (shiftEndTime - now) / (1000 * 60); 
...
module.exports = { assignCrew, checkForNewBuses, updateShiftStatuses };
